...
class Multimodal_SAMVAE_cr(VariationalAutoencoderMultimodal):
    def __init__(self, params):
        ...
        # Decoder for the risk probabilities
        self.time_probs = Decoder(latent_dim=self.latent_dim_total,
                                  hidden_size=self.time_hidden_size,
                                  feat_dists=[('categorical',self.n_times)],
                                  modes=self.time_mode,
                                  N_wsi = self.N_wsi,
                                  device=self.device)  # Estimator of the probability of occurence of each risk
        # Send all to device
        self.to(self.device)
    ...
    def forward(self, input_data):
        ...
        for i, data in enumerate(input_data):
            ...
            # MSample the latent space using reparameterization
        ...
        # Time decoder
        time_params = [self.Time_Decoder[i](z_agg) for i in range(self.n_times)]
        for i, param in enumerate(time_params):
            check_nan_inf(param, f'Time Decoder {i}')  
        
        # Risk probs
        risk_probs = self.time_probs(z_agg).detach().to(self.device)
    ...
    def calculate_risk(self, time_train, x_val, censor_val, time_val):  
        ...
        pred_risk_all = []
        ci_confident_intervals = [] 
        ibs_confident_intervals = []
        
        out_data = self(x_val)
        for i in range(self.n_times):  # For each risk, note that we are computing CIFs
            time_params = out_data['time_params']
            pred_risk = torch.zeros((time_params[i].shape[0], len(times)), device = self.device)
        
            for sample in range(pred_risk.shape[0]):
                if self.time_dists[i][0] == 'weibull':
                    alpha, lam = time_params[i][sample, 0], time_params[i][sample, 1]
                    prob = out_data['risk_probs'][sample, i]
                    pred_risk[sample, :] = (1 - torch.exp(-torch.pow(times / lam, alpha))) * prob
            ...
            for i, (x, censor) in enumerate(zip(time_val, censor_val)):
                ...
                ci_confident_intervals.append([bern_conf_interval(len(x), ci) for x, ci in zip(x_val, ci_list)])
                ibs_confident_intervals.append([bern_conf_interval(len(x), ibs, ibs=True) for x, ibs in zip(x_val, ibs_list)])
                pred_risk_all.append(pred_risk_np)  
            
        return ci_confident_intervals, ibs_confident_intervals, pred_risk_all, pred_risk_np, times_np
    ...
    def fit(self, data, train_params):
        ...
        for epoch in range(train_params['n_epochs']):
            ...
            # Compute metrics
            epoch_results['ci_va'], epoch_results['ibs_va'],_,_,_ = self.calculate_risk(time_train, cov_val, censor_val, time_val) 
            ...
            if epoch % 50 == 0:
                print('Iteration = ', epoch,
                      ...
                      '; C-index = ', '{:.4f} / {:.4f}'.format(np.mean(epoch_results['ci_va'][0]), np.mean(epoch_results['ci_va'][1])),
                      '; IBS = ', '{:.4f} / {:.4f}'.format(np.mean(epoch_results['ibs_va'][0]), np.mean(epoch_results['ibs_va'][1]))
                      )
        ...
        # Set the best ci and ibs in the last epoch of the validation, just to ensure that they are taken into account when selecting the best seed after
        ...
        _, _, training_stats['final_cif'],_,_ = self.calculate_risk(time_train, cov_val, censor_val, time_val)
