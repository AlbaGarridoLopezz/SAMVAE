...
from plots import (
    plot_multiple_models, plot_model_losses, plot_survival,
    plot_model_C_index, plot_model_IBS, plot_CIF,
    plot_interactive_models, load_existing_fold_results, find_log_names
)
...
def train_multimodal(data, feat_distributions, params, seed, output_dir, args, fold):
    ...
    # Plot CIF  
        ...
        plot_CIF(cif_samvae=training_results['final_cif'], df_train=[data[-1][0][:, 0]], df_val=[d[2] for d in data[:-1]], 
...
def main():
    print(Fore.RED + '\n\n-------- Survival Analysis Multimodal model based on Variational Autoencoders (SAMVAE)  --------' + Style.RESET_ALL)
    ...
    if args['n_seeds'] == 10 and args['final_plots']:      
        ...
        plot_multiple_models(multiple_survival_path, args['competing_risks']) 
        plot_interactive_models(multiple_survival_path)
